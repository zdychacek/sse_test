...
EventStream.prototype._writeEventData = function (res, options /* event, data, id, retry */) {
	var buffer = [];

	options || (options = {});

	options.retry && buffer.push('retry: ' + options.retry);
	(typeof options.id !== 'undefined') && buffer.push('id: ' + options.id);
	options.event && buffer.push('event: ' + options.event);
	(typeof options.data !== 'undefined') && buffer.push('data: ' + JSON.stringify(options.data));
